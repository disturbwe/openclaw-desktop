...
// Import getMainWindow from index.js for navigation/screenshot APIs
const { getMainWindow } = require('./index');

const isWindows = process.platform === 'win32';

let PORT = 7000;
let OPENCLAW_DIR = path.join(os.homedir(), '.openclaw');
let WORKSPACE_DIR = os.homedir();
let AGENT_ID = 'main';
let sessDir, cronFile, dataDir, memoryDir, memoryMdPath, heartbeatPath, healthHistoryFile;
...
function initPaths(options = {}) {
  ...
  OPENCLAW_DIR = options.openclawDir || path.join(os.homedir(), '.openclaw');
  WORKSPACE_DIR = options.workspaceDir || os.homedir();
  AGENT_ID = options.agentId || 'main';

  sessDir = path.join(OPENCLAW_DIR, 'agents', AGENT_ID, 'sessions');
  cronFile = path.join(OPENCLAW_DIR, 'cron', 'jobs.json');
  dataDir = path.join(WORKSPACE_DIR, 'data');
  memoryDir = path.join(WORKSPACE_DIR, 'memory');
  memoryMdPath = path.join(WORKSPACE_DIR, 'MEMORY.md');
  heartbeatPath = path.join(WORKSPACE_DIR, 'HEARTBEAT.md');
  healthHistoryFile = path.join(dataDir, 'health-history.json');
  ...
  workspaceFilenames = ['AGENTS.md','HEARTBEAT.md','IDENTITY.md','MEMORY.md','SOUL.md','TOOLS.md','USER.md'];
  claudeUsageFile = path.join(dataDir, 'claude-usage.json');
  geminiUsageFile = path.join(dataDir, 'gemini-usage.json');
  scrapeScript = path.join(WORKSPACE_DIR, 'scripts', 'scrape-claude-usage.sh');
  geminiScrapeScript = path.join(WORKSPACE_DIR, 'scripts', 'scrape-gemini-usage.sh');
  pricingFile = path.join(WORKSPACE_DIR, 'data', 'model_pricing_usd_per_million.json');

  htmlPath = options.htmlPath || path.join(__dirname, '../renderer/index.html');
}
...
function getSessionsJson() {
  try {
    ...
    return Object.entries(data).map(([key, s]) => ({
      ...
      status: s.abortedLastRun ? 'aborted' : 'completed',
      startTime: new Date((s.createdAt || s.updatedAt || 0) * 1000).toISOString(),
      messageCount: 0,
      tokensUsed: s.totalTokens || 0,
      cost: getSessionCost(s.sessionId || key),
      snippet: getLastMessage(s.sessionId || key)
    }));
  } catch (e) { return []; }
}
...
function getSystemStats() {
  try {
    ...
    let cpuTemp = null;
    if (isWindows) {
      // Windows: CPU temp requires WMI with admin rights, skip for now
      cpuTemp = null;
    ...
    try {
      if (isWindows) {
        const out = execSync('powershell -NoProfile -Command "Get-CimInstance Win32_LogicalDisk -Filter \\"DriveType=3\\" | Select-Object Size,FreeSpace | ConvertTo-Json"', { encoding: 'utf8', timeout: 5000 });
        let disks = [];
        try {
        ...
        if (disks.length > 0) {
          ...
          for (const d of disks) {
            ...
            totalFree += parseInt(d.FreeSpace) || 0;
          }
          const usedBytes = totalSize - totalFree;
          diskPercent = totalSize > 0 ? Math.round((usedBytes / totalSize) * 100) : 0;
          diskUsed = (usedBytes / 1073741824).toFixed(0) + 'G';
          diskTotal = (totalSize / 1073741824).toFixed(0) + 'G';
        }
      } else if (process.platform === 'darwin') {
    ...
    let crashesToday = 0;
    if (isWindows) {
      // Windows: No journalctl equivalent, skip crash counting
    } else if (process.platform === 'linux') {
      try {
        let logs = '';
        try {
  ...
  try {
    ...
    for (const repo of repos) {
      try {
        ...
        const gitCmd = isWindows
          ? `git -C "${repo.path}" log --oneline --since="7 days ago" -10 --format="%H|%s|%at"`
          : `git -C ${repo.path} log --oneline --since='7 days ago' -10 --format='%H|%s|%at'`;
        const log = execSync(gitCmd, { encoding: 'utf8', timeout: 5000 }).trim();
        if (!log) continue;
        log.split('\n').forEach(line => {
...
function getServicesStatus() {
  ...
  if (isWindows) {
    const hasWindowsProcess = (processName) => {
      try {
        const out = execSync(`powershell -NoProfile -Command "Get-Process -Name '${processName}' -ErrorAction SilentlyContinue | Select-Object -First 1"`, { encoding: 'utf8', timeout: 3000 });
    ...
    return services.map(name => {
      if (name === 'tailscaled') {
        return { name, active: hasWindowsProcess('tailscale') || hasWindowsProcess('tailscaled') };
      }
      if (name === 'openclaw') {
        return { name, active: hasWindowsProcess('node') };
      }
...
function saveHealthSnapshot() {
  if (!healthHistoryFile) return; // Not initialized yet
  try {
...
// Health snapshot interval is started in startServer()

// Session cleanup removed - auth system disabled
...
const server = http.createServer((req, res) => {
  ...
  // Don't set restrictive security headers for control-ui (it needs iframe embedding)
  const isControlUi = req.url.startsWith('/control-ui/');
  if (!isControlUi) {
    setSecurityHeaders(res);
  }
  ...
  if (req.url === '/api/auth/status') {
    ...
    res.end(JSON.stringify({ registered: false, loggedIn: true }));
    return;
  }
  ...
  // 路由导航端点
  if (req.url === '/api/navigate' && req.method === 'POST') {
    setSecurityHeaders(res);
    let body = '';
    req.on('data', chunk => { body += chunk.toString(); });
    req.on('end', () => {
      try {
        const { path: routePath } = JSON.parse(body);
        const mainWindow = getMainWindow();
        if (mainWindow && mainWindow.webContents) {
          mainWindow.webContents.send('navigate', { path: routePath });
          res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.url.startsWith('/control-ui/')) {
    const controlUiDir = path.join(__dirname, '../../public/control-ui');
    let filePath = req.url.slice('/control-ui/'.length);
    if (!filePath || filePath === '/') filePath = 'index.html';

    // Security: prevent directory traversal
    const resolvedPath = path.resolve(controlUiDir, filePath);
    if (!resolvedPath.startsWith(controlUiDir)) {
      res.writeHead(403, { 'Content-Type': 'text/plain' });
      res.end('Forbidden');
      return;
    }
    ...
    try {
      if (!fs.existsSync(resolvedPath)) {
        ...
        const indexPath = path.join(controlUiDir, 'index.html');
        if (fs.existsSync(indexPath)) {
          const html = fs.readFileSync(indexPath, 'utf8');
          // Remove frame-ancestors restriction to allow iframe embedding
          res.setHeader('Content-Security-Policy', "default-src 'self'; base-uri 'none'; object-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' ws: wss:;");
          res.writeHead(200, { 'Content-Type': 'text/html' });
          res.end(html);
        } else {
      ...
      // Set CSP without frame-ancestors restriction
      res.setHeader('Content-Security-Policy', "default-src 'self'; base-uri 'none'; object-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' ws: wss:;");
  ...
  if (req.url.startsWith('/api/')) {
    ...
    if (req.url === '/api/sessions') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ sessions: getSessionsJson() }));
      return;
    }
    ...
    if (req.url === '/api/openclaw-config' && req.method === 'GET') {
      if (!requireAuth(req, res)) return;
      const configPath = process.env.OPENCLAW_CONFIG || path.join(OPENCLAW_DIR, 'openclaw.json');
      try {
    ...
    if (req.url === '/api/openclaw-config' && req.method === 'PUT') {
      ...
      req.on('end', () => {
        try {
          ...
          // Windows 不支持 systemctl，使用 npx openclaw 命令
          if (isWindows) {
            try { execSync('npx openclaw gateway restart', { timeout: 15000, shell: true }); } catch(e) { console.log('Gateway restart failed:', e.message); }
          } else {
    ...
    if (req.url === '/api/sys-security') {
      if (isWindows) {
        ...
        res.end(JSON.stringify({
          ufw: 'Not available on Windows',
          ports: 'Use: netstat -an or Get-NetTCPConnection',
          fail2ban: 'Not available on Windows',
          ssh: 'Not available on Windows',
          audit: 'Not available on Windows'
        }));
        return;
      }
    ...
    if (req.url.startsWith('/api/logs?')) {
      try {
        ...
        if (process.platform !== 'linux') {
          ...
          if (isWindows) {
            res.end('Logs (journalctl) are only available on Linux.\nOn Windows, use Event Viewer or check OpenClaw log files directly.');
          } else {
    ...
    if (req.url === '/api/action/restart-openclaw' && req.method === 'POST') {
      try {
        ...
        if (isWindows) {
          // Windows 使用 npx openclaw 命令
          exec('npx openclaw gateway restart', { shell: true }, (err, stdout, stderr) => {
            if (err) {
              console.log('Gateway restart failed:', err.message);
            }
          });
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true, message: '正在重启 OpenClaw Gateway...' }));
          return;
        }
    ...
    if (req.url === '/api/action/restart-dashboard' && req.method === 'POST') {
      try {
        ...
        if (isWindows) {
          // Windows 上重启 dashboard - 创建新进程后退出当前进程
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true, message: '正在重启 Dashboard...' }));
          setTimeout(() => {
    ...
    if (req.url === '/api/action/restart-tailscale' && req.method === 'POST') {
      ...
      if (isWindows) {
        exec('tailscale down && tailscale up', (err) => {
          res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (req.url === '/api/action/kill-tmux' && req.method === 'POST') {
      if (isWindows) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: false, message: 'tmux not available on Windows' }));
        return;
      }
    ...
    if (req.url === '/api/action/gc' && req.method === 'POST') {
      if (isWindows) {
        exec('git -C "' + WORKSPACE_DIR + '" gc --quiet 2>nul', (err) => {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true }));
        });
      } else {
    ...
    if (req.url === '/api/action/sys-update' && req.method === 'POST') {
      ...
      if (isWindows) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: false, message: 'System update not available on Windows. Use Windows Update.' }));
        return;
      }
    ...
    if (req.url === '/api/action/disk-cleanup' && req.method === 'POST') {
      if (isWindows) {
        exec('powershell -NoProfile -Command "Get-ChildItem -Path $env:TEMP -Recurse -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue; Write-Host Cleanup done"', { timeout: 60000 }, (err, stdout) => {
          res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (req.url === '/api/tailscale') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      try {
        const statusJson = execSync('tailscale status --json 2>nul', { encoding: 'utf8', timeout: 5000 });
        const status = JSON.parse(statusJson);
        const self = status.Self || {};
        const peers = Object.values(status.Peer || {}).filter(p => p.Online).length;
        let routes = [];
        try {
          const serveStatus = execSync('tailscale serve status 2>nul', { encoding: 'utf8', timeout: 3000 });
...
function startServer(options = {}) {
  ...
  // Start health snapshot monitoring
